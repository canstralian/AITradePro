...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    ws.on('message', async data => {
      try {
        ...
        if (message.type === 'ai_query') {
          ...
          const response = await aiAnalysisService.processNaturalLanguageQuery(
            message.query
          );

          // Also perform enhanced RAG analysis
          const ragAnalysis = await vectorStoreService.performRAGAnalysis(
            message.query,
            'BTC'
          );
  ...
  app.get(
    '/api/assets/:id',
    validateSchema(schemas.assetParams),
    validateSymbol,
    async (req, res) => {
      try {
  ...
  app.get(
    ...
    validateSchema(schemas.userParams),
    async (req, res) => {
      try {
  ...
  app.post(
    '/api/workers/enqueue',
    validateSchema(schemas.workerTask),
    async (req, res) => {
      try {
  ...
  try {
    ...
    logger.warn('Failed to generate historical data', {
      error: error instanceof Error ? error.message : error,
    });
  }
