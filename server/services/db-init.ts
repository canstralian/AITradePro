...
const mockInsights = [
  {
    ...
    description:
      'BTC is showing a strong bull flag pattern with potential breakout target of $52,000',
    confidence: 0.85,
  ...
  },
  {
    ...
    description:
      'Increased network activity and DeFi usage indicate positive momentum for ETH',
    confidence: 0.78,
    metadata: JSON.stringify({
...
const mockNews = [
  {
    ...
    summary:
      'Large corporation allocates 5% of treasury reserves to Bitcoin, citing inflation hedge',
    source: 'CryptoNews',
  ...
  },
  {
    ...
    summary:
      'Transaction volumes on L2 networks reach all-time high, reducing main network congestion',
  ...
  try {
    ...
    try {
      for (const asset of mockAssets) {
        await db
          .insert(
            db.schema?.assets ||
              (await import('../../shared/schema-sqlite')).assets
          )
          .values({
            ...asset,
            updatedAt: new Date(),
    ...
    try {
      for (const user of mockUsers) {
        await db
          .insert(
            db.schema?.users ||
              (await import('../../shared/schema-sqlite')).users
          )
          .values({
            ...user,
            createdAt: new Date(),
    ...
    try {
      for (const insight of mockInsights) {
        await db
          .insert(
            db.schema?.aiInsights ||
              (await import('../../shared/schema-sqlite')).aiInsights
          )
          .values({
            ...insight,
            createdAt: new Date(),
    ...
    try {
      for (const news of mockNews) {
        await db
          .insert(
            db.schema?.newsItems ||
              (await import('../../shared/schema-sqlite')).newsItems
          )
          .values({
            ...news,
            createdAt: new Date(),
    ...
    try {
      for (const asset of mockAssets.slice(0, 3)) {
        ...
        for (let i = 0; i < 24; i++) {
          ...
          await db
            .insert(
              db.schema?.marketData ||
                (await import('../../shared/schema-sqlite')).marketData
            )
            .values({
              id: nanoid(),
              assetId: asset.id,
