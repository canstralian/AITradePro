...
function initializeDatabase() {
  ...
  if (databaseUrl.startsWith('sqlite:') || databaseUrl.endsWith('.db') || databaseUrl === ':memory:') {
    // SQLite connection
  ...
  try {
    ...
    logger.error('Failed to connect to PostgreSQL, falling back to SQLite', { error });
...
// Lazy initialization
let dbInstance: ReturnType<typeof drizzle> | ReturnType<typeof sqliteDrizzle> | null = null;
...
export const db = new Proxy({} as any, {
  get(target, prop) {
    return getDatabase()[prop as keyof typeof db];
  }
});
