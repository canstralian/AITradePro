...
import { marketData, assets } from '../../shared/schema';
...
export class BacktestEngine {
  ...
  private async processBar(bar: MarketBar): Promise<void> {
    ...
    const signal = await this.strategy.onBar(bar, this.broker, this.config.symbol);
  ...
  private async fetchHistoricalData(): Promise<MarketBar[]> {
    ...
    // Query market data filtered by symbol
    // First get the asset by symbol, then filter market data by assetId
    const data = await (db as any).select({
      ...
      .from(marketData)
      .innerJoin(assets, eq(marketData.assetId, assets.id))
      .where(
        and(
          eq(assets.symbol, this.config.symbol),
