...
export default function WorkerQueue() {
  ...
  useEffect(() => {
    ...
    const unsubscribeResults = subscribe(
      'worker_result',
      (data: TaskResult) => {
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                ...
                <SelectContent className="trading-panel border-trading-border">
                  ...
                  <SelectItem value="news_correlation">
                    News Correlation
                  </SelectItem>
                  <SelectItem value="pattern_matching">
                    Pattern Matching
                  </SelectItem>
                    ...
                    {task.success && task.result && (
                      <div className="text-xs trading-muted">
                        {task.result.analysis?.confidenceScore && (
                          <span>
                            Confidence: {task.result.analysis.confidenceScore}%
                          </span>
                        )}
                        {task.result.overallSentiment && (
                          <span>
                            Sentiment:{' '}
                            {(task.result.overallSentiment * 100).toFixed(0)}%
                          </span>
                        )}
                      </div>
                    )}
